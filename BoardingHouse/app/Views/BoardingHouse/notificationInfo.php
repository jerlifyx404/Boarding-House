...
<div class="col-sm-12 text-center">
  <h4><small>Notifications for <?= esc($TenantInfo[0]['tenantName'] ?? 'Tenant') ?></small></h4>
  ...
  <div style="margin: 35px auto 25px auto; width: 90%; display: flex; align-items: center; justify-content: space-between; gap: 15px;">
    ...
    <a href="<?= base_url('BoardingHouse/notification') ?>" class="icon-btn" title="Back to Notifications" aria-label="Back to Notifications">
    ...
    <table class="center">
      ...
      <tbody id="tenantTable">
        <?php if (empty($TenantInfo)): ?>
          <tr>
            <td colspan="7">No rental requests found for this tenant.</td>
          </tr>
        <?php else: ?>
              ...
              <td>
                <div class="action-buttons">
                  <?php if ($Tenant['status'] == 'pending'): ?>
                    <a href="<?= base_url('BoardingHouse/approveRequest/' . esc($Tenant['requestID'])) ?>" class="icon-btn" title="Approve" onclick="return confirm('Are you sure you want to approve this request?');">
                      <i class="fas fa-check-circle"></i>
                    </a>
                    <a href="<?= base_url('BoardingHouse/declineRequest/' . esc($Tenant['requestID'])) ?>" class="icon-btn" title="Decline" onclick="return confirm('Are you sure you want to decline this request?');">
